...
class DualInputInvoiceAI:
    def __init__(self, google_api_key: str = None, db_path: str = "invoice_management.db"):
        ...
        # Database connection - create fresh DB without mock data
        self.db = InvoiceDatabase(db_path)
        self._clean_mock_data()
        ...
        print(f"   Database: {db_path} (cleaned of mock data)")
        print(f"   LLM: {'Google Gemini 1.5 Flash' if self.llm else 'Rule-based processing'}")
    
    def _clean_mock_data(self):
        """Remove all mock/sample data from database"""
        cursor = self.db.conn.cursor()
        
        # Delete all existing data (keeps schema)
        ...
        self.db.conn.commit()
        print("✅ Cleaned all mock data from database")
    ...
    def _build_processing_graph(self) -> StateGraph:
        ...
        workflow.add_edge("parse_inputs", "extract_entities")
    ...
    def _extract_with_ai_dual(self, textract_json: Dict, ocr_json: Dict) -> ExtractedInvoiceData:
        """Use AI to extract structured data from both Textract JSON and OCR text"""
        
        extraction_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert invoice processing AI. Extract structured information from BOTH Textract JSON data AND raw OCR text.
...
- Cross-reference Textract structured data with OCR text for accuracy"""),
    ...
    def _extract_with_rules_dual(self, textract_json: Dict, ocr_json: Dict) -> ExtractedInvoiceData:
                ...
                for row in rows[1:]:
                    if len(row) >= 4:
                        ...
                        if line_item["description"] and line_item["description"] != "Total":
                            # Validate with OCR text
                            if line_item["hsn_code"] and line_item["hsn_code"] in ocr_text:
    ...
    def _store_database_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            # 1. Insert document with OCR text
            cursor.execute("""
    ...
    def process_dual_inputs(self, textract_json_path: str, ocr_json_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Failed to load input files: {e}"}
